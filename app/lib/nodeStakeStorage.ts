...
class NodeStakeStorage {
  ...
  async saveNodeStakeData(publicKey: string, data: Omit<NodeStakeData, 'createdAt' | 'expiresAt'>): Promise<void> {
    try {
      ...
      await Storage.set({ key: getStorageKey(publicKey), value: JSON.stringify(stakeData) })
  ...
  async getNodeStakeData(publicKey: string): Promise<NodeStakeData | null> {
    try {
      const { value } = await Storage.get({ key: getStorageKey(publicKey) })
      
      if (!value) {
        return null
      }
      const data = JSON.parse(value) as NodeStakeData
  ...
  async deleteNodeStakeData(publicKey: string): Promise<void> {
    try {
      await Storage.remove({ key: getStorageKey(publicKey) })
