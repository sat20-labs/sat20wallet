          ...
          <div class="flex-1">
            ...
            <div class="flex items-center justify-between mt-1">
              <div class="text-sm text-zinc-500">
                {{ $t('assetOperationDialog.currentBalance') }}
              </div>
            ...
            <!-- 操作提示 -->
            <div v-if="getOperationHint()" class="mt-2 text-xs text-blue-400 flex items-center gap-1">
...
// 获取资产图标
const getAssetIcon = () => {
  if (props.assetTicker) {
    return props.assetTicker.charAt(0).toUpperCase()
  }
  if (props.assetKey) {
    return props.assetKey.charAt(0).toUpperCase()
  }
  return 'A'
}
...
const getAssetDisplayName = () => {
  if (props.assetTicker) {
    const ticker = props.assetTicker.toUpperCase()
    return ticker === '::' ? 'BTC' : ticker
  }
  if (props.assetKey) {
    const key = props.assetKey.toUpperCase()
    return key === '::' ? 'BTC' : key
  }
...
const getAssetType = () => {
  if (props.assetType) {
    return props.assetType.toUpperCase()
  }
  if (props.assetKey?.includes('runes')) {
    return 'RUNES'
  }
  if (props.assetKey?.includes('sat20')) {
    return 'SAT20'
  }
  return 'TOKEN'
}
...
const getOperationHint = () => {
  ...
  if (props.operationType === 'withdraw') {
    return 'SatoshiNet → Bitcoin'
  }
  return null
}
