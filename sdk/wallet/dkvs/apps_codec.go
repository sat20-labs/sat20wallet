...
const (
	...
	// CodecVersion identifies the compact DKVS application payload format.
	CodecVersion = dkvsAppPayloadVersion
	// CodecMaxFieldSize is the maximum compact text-field size.
	CodecMaxFieldSize = dkvsAppMaxText
	// CodecMaxPayloadSize is the maximum encrypted/ciphertext field size.
	CodecMaxPayloadSize = dkvsAppMaxCiphertext
)
...
func DecodeOfflineMessage(data []byte) (*DKVSOfflineMessage, error) {
...
func EncodeServiceAuthenticity(value DKVSServiceAuthenticity) ([]byte, error) {
...
func DecodeServiceAuthenticity(data []byte) (*DKVSServiceAuthenticity, error) {
