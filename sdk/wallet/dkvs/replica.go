...
const ReplicaVersion = byte(2)

var (
	ReplicaConfirmedPrefix = []byte("dkvs-replica-confirmed-v1:")
	ReplicaRootPrefix      = []byte("dkvs-replica-root-v1:")
)
...
func ReplicaScope(namespace string, filters []dkvsindexer.Subscription) string {
...
func ReplicaRecordKey(base []byte, scope, recordKey string) []byte {
	prefix := dkvsReplicaPrefix(base, scope)
	hash := sha256.Sum256([]byte(recordKey))
	return append(prefix, hash[:]...)
}

func (s *ReplicaStore) loadRecords(base []byte, scope string) ([]*swire.DKVSRecord, error) {
...
func (s *ReplicaStore) LoadConfirmed(scope string) ([]*swire.DKVSRecord, error) {
	return s.loadRecords(ReplicaConfirmedPrefix, scope)
}

func (s *ReplicaStore) LoadBaseline(scope string) (*ReplicaBaseline, error) {
	if s == nil || s.db == nil || scope == "" {
		return nil, fmt.Errorf("DKVS replica store is unavailable")
	}
	value, err := s.db.Read(append(append([]byte(nil), ReplicaRootPrefix...), scope...))
	if err != nil {
		return nil, err
	}
	if len(value) != 1+2*chainhash.HashSize+8 || value[0] != ReplicaVersion {
		return nil, dkvsindexer.ErrInvalidRecord
	}
	baseline := &ReplicaBaseline{}
...
func (s *ReplicaStore) LoadRoot(scope string) (string, error) {
...
func (s *ReplicaStore) ApplyConfirmed(scope string, filters []dkvsindexer.Subscription,
	...
	for _, record := range existing {
		if err := batch.Delete(ReplicaRecordKey(ReplicaConfirmedPrefix, scope, record.Key)); err != nil {
			return err
		}
	}
	for _, record := range incoming {
		...
		if err := batch.Put(ReplicaRecordKey(ReplicaConfirmedPrefix, scope, record.Key), encoded); err != nil {
			return err
		}
	}
	rootValue := make([]byte, 1+2*chainhash.HashSize+8)
	rootValue[0] = ReplicaVersion
	...
	if err := batch.Put(append(append([]byte(nil), ReplicaRootPrefix...), scope...), rootValue); err != nil {
		return err
	}
	return batch.Flush()
}
