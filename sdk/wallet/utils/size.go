...
const (
	...
	//	- CountTxOut: 1 byte
	//	- TxOut: 74 + 43  bytes
	//		OutputPayingToThem, //p2tr
	//		OutputPayingToUs, 
	//	- LockTime: 4 bytes
	BaseCommitmentTxSize2 = 4 + 1 + FundingInputSize + 1 +
		CommitmentDelayOutput + TaprootCommitmentAnchorOutput + 4
...
func (twe *TxWeightEstimator) AddTapscriptInput(
	...
	twe.inputSize += InputSize
	twe.inputWitnessSize += leafWitnessSize + int64(
...
func (twe *TxWeightEstimator) AddOutput(pkScript []byte) *TxWeightEstimator {
	twe.outputSize += BaseOutputSize + int64(len(pkScript))
	twe.outputCount++
...
func (twe *TxWeightEstimator) Weight() int64 {
	...
	txSizeStripped := BaseTxSize + int64(inputCount) +
		twe.inputSize + int64(outputCount) + twe.outputSize
	weight := int64(txSizeStripped * witnessScaleFactor)
...
func (twe *TxWeightEstimator) VSize() int64 {
	...
	return int64(math.Ceil(float64(twe.Weight()) / 4))
}

// feeRate: sat/vb
func (twe *TxWeightEstimator) Fee(feeRate int64) int64 {
	vSize := twe.VSize()
	return SatPerVByte(feeRate).FeePerKVByte().FeeForVSize(vSize)
}

func CalcFee_CommitTx(in, out int, feeRate int64) int64 {
