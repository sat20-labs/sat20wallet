<template>
  <LayoutApprove :loading="isLoading" :confirm-disabled="!canSign" @confirm="confirm" @cancel="cancel">
    <div class="space-y-4">
...
const canSign = computed(() => Boolean(
  !isLoading.value && !parseError.value && parsedInputs.value.length > 0 &&
  parsedOutputs.value.length > 0 && props.metadata?.identityGeneration,
))

const parseTxDetailItems = (items: unknown): TxDetailItem[] => {
...
const dedupeInputsByOutpoint = (items: TxDetailItem[]): TxDetailItem[] => {
...
watch(() => [props.data?.psbtHex, props.data.options?.chain, network.value, props.metadata?.identityGeneration], async (_value, _oldValue, onCleanup) => {
  // Ignore results from an approval that was replaced while loading.
  let cancelled = false
  onCleanup(() => { cancelled = true })
  const identityGeneration = props.metadata?.identityGeneration
  ...
  try {
    ...
    if (err) {
      ...
      try {
        if (res.inputs) {
          parsedInputs.value = dedupeInputsByOutpoint(parseTxDetailItems(res.inputs))
...
const confirm = async () => {
  ...
  try {
      assertWalletIdentityReady(identityGeneration)
      if (['sat20', 'satnet', 'satsnet'].includes(options.chain ?? '')) {
