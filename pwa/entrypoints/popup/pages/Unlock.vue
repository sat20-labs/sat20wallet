...
const checkBiometricSupport = async () => {
  try {
    ...
    if (supportResult.supported && supportResult.available) {
      // 检查是否有活跃的生物识别凭据
      const hasCredentials = await biometricCredentialManager.hasActiveCredentials()
...
onMounted(async () => {
  ...
  // 如果支持生物识别且已创建加密凭据，默认显示生物识别界面
  if (showBiometricButton.value && await biometricCredentialManager.hasActiveCredentials()) {
