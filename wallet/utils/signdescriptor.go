...
// 定义一个类型来封装 btcec.PublicKey 并实现 gob 接口
type GobPubKey struct {
	*btcec.PublicKey
}

func NewGobPubKey(k *btcec.PublicKey) *GobPubKey {
	return &GobPubKey{
		PublicKey: k,
	}
}
...
// 实现 GobEncoder 接口
func (k *GobPubKey) GobEncode() ([]byte, error) {
	if k.PublicKey == nil {
		return []byte{}, nil
	}
	// 序列化 X 和 Y 坐标
...
// 实现 GobDecoder 接口
func (k *GobPubKey) GobDecode(data []byte) error {
	if len(data) == 0 {
		k.PublicKey = nil
		return nil
	}
...
// GobEncode 为 KeyDescriptor 实现 GobEncoder 接口
func (kd *KeyDescriptor) GobEncode() ([]byte, error) {
	var buf bytes.Buffer
	enc := gob.NewEncoder(&buf)

	// 编码 KeyLocator
	if err := enc.Encode(kd.KeyLocator); err != nil {
		return nil, err
	}
	...
	if kd.PubKey != nil {
		...
		if err := enc.Encode(pubKeyBytes); err != nil {
			return nil, err
		}
	} else {
		// 如果 PubKey 为空，编码空字节切片
...
// GobDecode 为 KeyDescriptor 实现 GobDecoder 接口
func (kd *KeyDescriptor) GobDecode(data []byte) error {
	buf := bytes.NewBuffer(data)
	dec := gob.NewDecoder(buf)

	// 解码 KeyLocator
	if err := dec.Decode(&kd.KeyLocator); err != nil {
		return err
	}
